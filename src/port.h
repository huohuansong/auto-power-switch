...
#define SET_POWER_STATUS(phy_port, onoff) do { \
		if (PORT_PHYSICAL_PORT_1 == phy_port) { \
			P3_2 = (PORT_POWER_OFF == onoff) ? 1 : 0; \
		} else if (PORT_PHYSICAL_PORT_2 == phy_port)  { \
			P3_3 = (PORT_POWER_OFF == onoff) ? 1 : 0; \
		} else if (PORT_PHYSICAL_PORT_3 == phy_port)  { \
			P5_4 = (PORT_POWER_OFF == onoff) ? 1 : 0; \
		} else { \
			\
		} \
	} while(0)
